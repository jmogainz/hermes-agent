...
def test_loop_type_compiles_to_set_value_on_chosen_target():
    ...
    assert result.ok is True
    assert result.status == "done"
    # This loop also probes `capture` through the handle; the chosen target must be written by
    # exactly one set_value (TYPE(target, text) never dispatches as a bare `type`).
    assert [c for c in calls if c.get("action") != "capture"] == [
        {"action": "set_value", "element": 2, "value": "Kevin"}]
